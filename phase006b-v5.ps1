...
function Invoke-Native {
  ...
  $stdout = Join-Path $logsRoot "phase006b-v5-$timestamp-out.log"
  $stderr = Join-Path $logsRoot "phase006b-v5-$timestamp-err.log"
  ...
  if ($process.ExitCode -ne 0) {
    $diagnostic = Join-Path $logsRoot "FAILED-phase006b-v5-$timestamp-$($Name.Replace(' ','-')).log"
...
function Get-PackageJsonValueViaNode {
  param(
...
"@
  ...
  $value = & node -e $script $PackageJsonPath $Section $Name

  if ($LASTEXITCODE -ne 0 -or [string]::IsNullOrWhiteSpace($value)) {
    throw "Could not resolve $Section.$Name from $PackageJsonPath"
  }
...
Write-Host "HIBEYA AKAL BUDI - PHASE 006B V5" -ForegroundColor Cyan
Write-Host "Shared React Primitives + Storybook" -ForegroundColor Cyan
Write-Host "Workspace-aware dependency resolution" -ForegroundColor Cyan
Write-Host "====================================================" -ForegroundColor Cyan


# ============================================================
# 1. Resolve dependency ranges from the actual workspace
#    package.json files. Do not assume React is hoisted to root.
# ============================================================
...
$typescriptRange =
  Get-PackageJsonValueViaNode `
    -PackageJsonPath $contentSchemaPackagePath `
    -Section "devDependencies" `
    -Name "typescript"
...
Write-Host "PHASE 006B V5: PASS" -ForegroundColor Green
Write-Host "====================================================" -ForegroundColor Green
Write-Host ""
