...
function Invoke-Native {
  ...
  $stdout =
    Join-Path $logsRoot "phase006b-v6-$timestamp-out.log"

  $stderr =
    Join-Path $logsRoot "phase006b-v6-$timestamp-err.log"
  ...
  if (
    ...
    $diagnostic =
      Join-Path `
        $logsRoot `
        "FAILED-phase006b-v6-$timestamp-$($Name.Replace(' ','-')).log"
...
function Get-JsonNestedString {
  ...
  foreach (
    ...
    if (
      ...
      throw "Could not resolve $Label. Missing property: $segment"
    }
  ...
  if (
    ...
    throw "Could not resolve $Label. Value is empty."
  }
...
Write-Host "HIBEYA AKAL BUDI - PHASE 006B V6" -ForegroundColor Cyan
Write-Host "Shared React Primitives + Storybook" -ForegroundColor Cyan
Write-Host "PowerShell-safe JSON dependency resolution" -ForegroundColor Cyan
Write-Host "====================================================" -ForegroundColor Cyan


# ============================================================
# 1. Resolve workspace dependency ranges safely using
#    PSObject.Properties instead of nested StrictMode access
#    and without node -e quoting.
# ============================================================
...
if (
  ...
  throw "Missing apps\learner-web\package.json"
}

if (
  -not (Test-Path $contentSchemaPackagePath)
) {
  throw "Missing packages\content-schema\package.json"
}
...
$reactRange =
  Get-JsonNestedString `
    -RootObject $learnerPackage `
    -Path @(
      "dependencies",
      "react"
    ) `
    -Label "learner-web dependencies.react"
...
$reactDomRange =
  Get-JsonNestedString `
    ...
    -Label "learner-web dependencies.react-dom"
...
$typesReactRange =
  Get-JsonNestedString `
    ...
    -Label "learner-web devDependencies.@types/react"
...
$typesReactDomRange =
  Get-JsonNestedString `
    ...
    -Label "learner-web devDependencies.@types/react-dom"

# TypeScript can live in dependencies OR devDependencies depending
# on the package template. Resolve both without assuming either.
$typescriptRange =
  $null
...
try {
  $typescriptRange =
    Get-JsonNestedString `
      -RootObject $contentSchemaPackage `
      -Path @(
        "devDependencies",
        "typescript"
      ) `
      -Label "content-schema devDependencies.typescript"
}
catch {
  $typescriptRange =
    Get-JsonNestedString `
      ...
      -Label "content-schema dependencies.typescript"
}
...
# ============================================================
# 2. Build package objects in one shot.
# ============================================================
...
# ============================================================
# 5. Root package scripts edited through a temporary JS file.
#    This avoids both PS StrictMode and node -e quoting issues.
# ============================================================
...
Write-Host "PHASE 006B V6: PASS" -ForegroundColor Green
Write-Host "====================================================" -ForegroundColor Green
Write-Host ""
