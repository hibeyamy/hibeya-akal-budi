...
$work=Join-Path $logs ("phase008r3b4-repair-v2-"+$stamp)
$log=Join-Path $work "repair.log"
$zip=Join-Path $logs ("phase008r3b4-repair-v2-"+$stamp+".zip")
...
trap{Log "";Log "PHASE 008R3B.4 REPAIR V2: FAILED";Log ($_|Out-String);Log $_.ScriptStackTrace;ZipLog;Write-Host ("ZIP: "+$zip) -ForegroundColor Yellow;exit 1}

Write-Host "====================================================" -ForegroundColor Cyan
Write-Host "HIBEYA AKAL BUDI - PHASE 008R3B.4 REPAIR V2" -ForegroundColor Cyan
Write-Host "Correct Production-Mechanic Test Sequencing" -ForegroundColor Cyan
Write-Host "====================================================" -ForegroundColor Cyan

$src=Join-Path $root "repair-phase008r3b4-v2-responsive.spec.ts"
$dst=Join-Path $root "tools\visual-regression\tests\learner-responsive-production.spec.ts"
if(-not(Test-Path $src)){throw "Repair V2 responsive spec missing"}
Copy-Item $src $dst -Force
Log "PASS: corrected responsive regression installed"

# V1 already rebuilt Storybook successfully with learner styling.
...
Log ""
Log "PHASE 008R3B.4 REPAIR V2: PASS"
Log "Responsive regression now discovers an incorrect option through fresh production-mechanic instances."
Log "The test no longer attempts interaction after a correct answer has completed and disabled the activity."
Log "Retry -> correct -> completed is exercised without hard-coded correctness."
...
ZipLog
Write-Host "PHASE 008R3B.4 REPAIR V2: PASS" -ForegroundColor Green
