...
function Invoke-Native {
  ...
  $stdout =
    Join-Path $logsRoot "phase006b-v3-$timestamp-out.log"

  $stderr =
    Join-Path $logsRoot "phase006b-v3-$timestamp-err.log"
  ...
  if (
    ...
    $diagnostic =
      Join-Path `
        $logsRoot `
        "FAILED-phase006b-v3-$timestamp-$($Name.Replace(' ','-')).log"
...
Write-Host "HIBEYA AKAL BUDI - PHASE 006B V3" -ForegroundColor Cyan
Write-Host "Shared React Primitives + Storybook" -ForegroundColor Cyan
Write-Host "Idempotent rebuild; no nested hashtable mutation" -ForegroundColor Cyan
Write-Host "====================================================" -ForegroundColor Cyan


# ============================================================
# 1. Resolve versions from existing workspaces.
# ============================================================

Write-Step "Resolving existing toolchain"
...
if (
  -not (Test-Path $learnerPackagePath)
) {
  throw "apps\learner-web\package.json is missing."
}

if (
  -not (Test-Path $contentSchemaPackagePath)
) {
  throw "packages\content-schema\package.json is missing."
}
...
$contentSchemaPackage =
  Get-Content $contentSchemaPackagePath -Raw |
  ConvertFrom-Json

$reactVersion =
  Get-StringProperty `
    -Object $learnerPackage.dependencies `
    -PropertyName "react"

$reactDomVersion =
  Get-StringProperty `
    -Object $learnerPackage.dependencies `
    -PropertyName "react-dom"

$typesReactVersion =
  Get-StringProperty `
    -Object $learnerPackage.devDependencies `
    -PropertyName "@types/react"

$typesReactDomVersion =
  Get-StringProperty `
    -Object $learnerPackage.devDependencies `
    -PropertyName "@types/react-dom"

$typescriptVersion =
  Get-StringProperty `
    -Object $contentSchemaPackage.devDependencies `
...
if (
  ...
  throw "Could not resolve React version."
}
...
if (
  ...
  throw "Could not resolve react-dom version."
}
...
if (
  ...
  throw "Could not resolve TypeScript version."
}
...
# ============================================================
# 2. Build dev-dependency maps BEFORE the package objects.
#    This avoids PowerShell 5.1 StrictMode nested-hashtable
#    assignment failures entirely.
# ============================================================
  ...
  @{
    ...
    "scripts" =
      @{
        ...
        "test" =
          "echo `"storybook stories validated by build`""
...
# ============================================================
# 3. Rebuild @akal-budi/ui deterministically.
# ============================================================

Write-Step "Writing @akal-budi/ui package"
...
# ============================================================
# 4. Rebuild Storybook workspace deterministically.
# ============================================================
...
# ============================================================
# 6. Install framework packages. This command is idempotent.
# ============================================================
...
Write-Host "PHASE 006B V3: PASS" -ForegroundColor Green
Write-Host "====================================================" -ForegroundColor Green
Write-Host ""
