...
function Invoke-Native {
  ...
  $stdout = Join-Path $logsRoot "phase006b-$timestamp-out.log"
  $stderr = Join-Path $logsRoot "phase006b-$timestamp-err.log"
  ...
  if ($process.ExitCode -ne 0) {
    $diagnostic = Join-Path $logsRoot "FAILED-phase006b-$timestamp-$($Name.Replace(' ','-')).log"
...
Write-Host "HIBEYA AKAL BUDI - PHASE 006B" -ForegroundColor Cyan
Write-Host "Shared React Primitives + Storybook" -ForegroundColor Cyan
Write-Host "====================================================" -ForegroundColor Cyan


# ------------------------------------------------------------
# Resolve React versions from learner-web so UI package stays
# aligned with the actual application toolchain.
# ------------------------------------------------------------

Write-Step "Resolving existing React toolchain"

$learnerPackagePath =
  Join-Path $repoRoot "apps\learner-web\package.json"

if (-not (Test-Path $learnerPackagePath)) {
  throw "apps\learner-web\package.json not found."
}

$learnerPackage =
  Get-Content $learnerPackagePath -Raw |
  ConvertFrom-Json

$reactVersion =
  [string]$learnerPackage.dependencies.react

$reactDomVersion =
  [string]$learnerPackage.dependencies.'react-dom'

$typesReactVersion =
  [string]$learnerPackage.devDependencies.'@types/react'

$typesReactDomVersion =
  [string]$learnerPackage.devDependencies.'@types/react-dom'

if ([string]::IsNullOrWhiteSpace($reactVersion)) {
  throw "React dependency version could not be resolved from learner-web."
}

if ([string]::IsNullOrWhiteSpace($reactDomVersion)) {
  throw "react-dom dependency version could not be resolved from learner-web."
}

Write-Host "React: $reactVersion" -ForegroundColor Green
Write-Host "react-dom: $reactDomVersion" -ForegroundColor Green
...
Write-Step "Creating @akal-budi/ui package"
...
if (-not [string]::IsNullOrWhiteSpace($typesReactVersion)) {
  $uiPackage["devDependencies"]["@types/react"] = $typesReactVersion
}

if (-not [string]::IsNullOrWhiteSpace($typesReactDomVersion)) {
  $uiPackage["devDependencies"]["@types/react-dom"] = $typesReactDomVersion
}

$uiPackage["devDependencies"]["typescript"] =
  [string](
    (
      Get-Content (Join-Path $repoRoot "packages\content-schema\package.json") -Raw |
      ConvertFrom-Json
...
.ab-ui-button:focus-visible {
  outline: var(--ab-focus-ring, #f59e0b) solid 4px;
  outline-offset: 3px;
}
...
Write-Step "Creating UI Storybook workspace"
...
if (-not [string]::IsNullOrWhiteSpace($typesReactVersion)) {
  $storybookPackage["devDependencies"]["@types/react"] = $typesReactVersion
}

if (-not [string]::IsNullOrWhiteSpace($typesReactDomVersion)) {
  $storybookPackage["devDependencies"]["@types/react-dom"] = $typesReactDomVersion
}
...
const meta = {
  ...
  args: {
    children:
      "Cuba lagi. Tengok dengan teliti."
  }
} satisfies Meta<
...
export const Error:
  Story = {
    args: {
      ...
      children:
        "Aktiviti belum dapat dimuatkan."
    }
  };
'@
...
Write-Step "Adding Storybook automation commands"

$rootPackagePath =
  Join-Path $repoRoot "package.json"

Backup-File $rootPackagePath

$rootPackage =
  Get-Content $rootPackagePath -Raw |
  ConvertFrom-Json
...
$rootPackage.scripts |
  Add-Member `
    -NotePropertyName "storybook" `
    -NotePropertyValue "pnpm --filter ui-storybook storybook" `
    -Force

$rootPackage.scripts |
  Add-Member `
    -NotePropertyName "storybook:build" `
    -NotePropertyValue "pnpm --filter ui-storybook build-storybook" `
    -Force
...
# ------------------------------------------------------------
# Install Storybook packages into the dedicated workspace.
# Official Storybook guidance uses @storybook/react-vite for
# React + Vite and @storybook/addon-a11y for accessibility.
# ------------------------------------------------------------

Invoke-Native `
  -Name "Workspace dependency refresh" `
  -Command "pnpm install"

Invoke-Native `
  -Name "Install Storybook React-Vite framework" `
...
Write-Host "PHASE 006B: PASS" -ForegroundColor Green
Write-Host "====================================================" -ForegroundColor Green
Write-Host ""
Write-Host "Run Storybook locally with:" -ForegroundColor White
Write-Host "  pnpm storybook" -ForegroundColor Cyan
Write-Host ""
Write-Host "Next: Phase 006C App Token Adoption." -ForegroundColor Cyan
