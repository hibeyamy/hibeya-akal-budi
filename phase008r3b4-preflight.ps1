...
$work = Join-Path $logs ("phase008r3b4-preflight-" + $stamp)
$report = Join-Path $work "phase008r3b4-preflight.txt"
$zip = Join-Path $logs ("phase008r3b4-responsive-qa-preflight-" + $stamp + ".zip")
...
trap {
    Log ""
    Log "PHASE 008R3B.4 PREFLIGHT: FAILED"
    Log ($_ | Out-String)
...
Write-Host "HIBEYA AKAL BUDI - PHASE 008R3B.4 PREFLIGHT" -ForegroundColor Cyan
...
Log "6. Verify all learner choice targets remain at least the current preferred touch size."
...
Log ""
Log "PHASE 008R3B.4 PREFLIGHT: PASS"
Log "No learner source, test source, dependency, content, asset or lockfile was modified."

Save-Zip

Write-Host ""
Write-Host "PHASE 008R3B.4 PREFLIGHT: PASS" -ForegroundColor Green
