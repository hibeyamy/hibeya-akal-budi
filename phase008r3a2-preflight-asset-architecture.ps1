...
# remove accidental paren
$reportContent = Get-Content $report -Raw
$reportContent = $reportContent.Replace(
    '2. Separate authoritative source/master assets from runtime delivery assets.")',
    '2. Separate authoritative source/master assets from runtime delivery assets.'
)
[IO.File]::WriteAllText($report,$reportContent,[Text.UTF8Encoding]::new($false))
