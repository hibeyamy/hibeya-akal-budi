...
/*
 * Discover an incorrect option by asking the production mechanic on a fresh
 * activity instance for each candidate. This avoids hard-coding correctness
 * and avoids continuing after a correct answer has completed/disabled the UI.
 */
async function reachRetryThroughProductionMechanic(page: Page) {
  ...
  while (true) {
    ...
    if (state === "incorrect") {
      await expect(page.getByTestId("activity-player")).toHaveAttribute("data-activity-state", "retry");
      await expect(feedback).toContainText("Cuba lagi.");
      return;
    }

    expect(state).toBe("correct");
    await expect(page.getByTestId("activity-player")).toHaveAttribute("data-activity-state", "completed");
    candidate += 1;
  }
}
